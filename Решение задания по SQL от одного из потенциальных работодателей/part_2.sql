...
/* Создал таблицу в которой провел отбор по нужным месяцам и просуммировал выручку по транспорту.
Отделил суммы меньшие чем 50 000 и извлек коды клиентов, подходящих под запрос.
*/
WITH 
revenue_may AS (SELECT Код_клиента,
                       SUM(Выручка_Транспорт) AS Суммарная_выручка_с_мая_по_июль
                FROM выручка
                WHERE  Месяц  IN ('2022-05','2022-06','2022-07')
                GROUP BY 1
                HAVING SUM(Выручка_Транспорт)<50000
                ),
/*точно так же поступил и с другими месяцами*/
revenue AS      (SELECT Код_клиента,
                        SUM(Выручка_Транспорт) AS Суммарная_выручка_с_февраля_по_апрель
                 FROM выручка
                 WHERE  Месяц  IN ('2022-02','2022-03','2022-04')
                 GROUP BY 1
                )

/* на вывод код клиента, его название, выручки за разные месяца и разница между ними*/
SELECT DISTINCT rm.Код_клиента,
       con.Наименование_клиента,
       rm.Суммарная_выручка_с_мая_по_июль,
       r.Суммарная_выручка_с_февраля_по_апрель,
       rm.Суммарная_выручка_с_мая_по_июль-r.Суммарная_выручка_с_февраля_по_апрель AS Разница,
       ROUND((rm.Суммарная_выручка_с_мая_по_июль::numeric / r.Суммарная_выручка_с_февраля_по_апрель)*100,2) AS Разница_в_процентах
       
       

/*объеденил коды из временной таблицы и подтянул таблицы с данными и контактами.
Далее поставил условие по по мебельной отрасли*/

FROM revenue_may rm JOIN revenue r ON(rm.Код_клиента=r.Код_клиента)
    
LEFT JOIN клиенты AS c ON rm.Код_клиента=c.Код_клиента
LEFT JOIN контакты AS con ON rm.Код_клиента=con.Код_клиента
LEFT JOIN выручка AS v ON rm.Код_клиента=v.Код_клиента
